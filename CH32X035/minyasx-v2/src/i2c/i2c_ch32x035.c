...
void I2C_init(void) {
    ...
    // PA11 (SDA) as alternate function open-drain, 10MHz
    GPIOA->CFGHR &= ~(0xF << (4 * (11 - 8)));
    GPIOA->CFGHR |= (GPIO_Speed_10MHz | GPIO_CNF_OUT_OD_AF) << (4 * (11 - 8));

    // PA10 (SCL) as alternate function open-drain, 10MHz
    GPIOA->CFGHR &= ~(0xF << (4 * (10 - 8)));
    GPIOA->CFGHR |= (GPIO_Speed_10MHz | GPIO_CNF_OUT_OD_AF) << (4 * (10 - 8));
