...
uint16_t PD_getPDOMinVoltage(uint8_t pdonum)
{
  ...
  if (pdonum <= ppspos)
    ...
    return PD_control.PPSSourceCap[pdonum - ppspos - 1].MinVoltage;
}
...
uint16_t PD_getPDOMaxVoltage(uint8_t pdonum)
{
  ...
  if (pdonum <= ppspos)
    ...
    return PD_control.PPSSourceCap[pdonum - ppspos - 1].MaxVoltage;
}
...
uint16_t PD_getPDOMaxCurrent(uint8_t pdonum)
{
  ...
  if (pdonum <= ppspos)
    ...
    return PD_control.PPSSourceCap[pdonum - ppspos - 1].Current;
}
...
uint8_t PD_connect(void)
{
  ...
  GPIOB->CFGHR = (GPIOB->CFGHR & ~((uint32_t)0b1111 << (((14) & 7) << 2) | (uint32_t)0b1111 << (((15) & 7) << 2))) | ((uint32_t)0b0100 << (((14) & 7) << 2) | (uint32_t)0b0100 << (((15) & 7) << 2));
#ifdef USB_VDD
#if USB_VDD > 0
