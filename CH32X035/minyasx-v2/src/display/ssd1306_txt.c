...
void OLED_clearLine(uint8_t y)
{
  ...
  for (i = OLED_WIDTH; i; i--)
    I2C_write(0x00); // clear line
  I2C_stop();        // stop transmission
...
void OLED_clear(void)
{
  ...
  while (y--)
    OLED_clearLine(y); // clear all lines
}
...
#if OLED_BIGCHARS > 0
  if (OLED_sz == 0)
  { // normal character (5x8)
#endif
    if (OLED_x > OLED_WIDTH - 6)
      OLED_cursor(0, OLED_y + 1);
    I2C_start(OLED_ADDR << 1);       // start transmission to OLED
    I2C_write(OLED_DAT_MODE);        // set data mode
    I2C_write(OLED_i ? 0xff : 0x00); // write space between characters
    for (uint8_t i = 5; i; i--)
      I2C_write(OLED_i ? ~OLED_FONT[ptr++] : OLED_FONT[ptr++]);
    I2C_stop();
...
void OLED_write(char c)
{
  ...
  if (c >= 32)
    OLED_plotChar(c); // normal character
#if OLED_BIGCHARS > 0
  else if (c == '\n')
...
void OLED_clearRect(uint8_t w, uint8_t h)
{
  ...
  while (h--)
  {
    ...
    I2C_write(OLED_DAT_MODE);  // set data mode
    for (uint8_t i = w; i; i--)
      I2C_write(OLED_i ? 0xff : 0x00); // clear line
    I2C_stop();                        // stop transmission
    OLED_cursor(OLED_x, OLED_y + 1);   // set next line
  }
  OLED_cursor(OLED_x + w, y); // move cursor
...
void OLED_printSegment(uint16_t value, uint8_t digits, uint8_t lead, uint8_t decimal)
{
  ...
  while (digits--)
  {                                     // for all digits digits
    uint8_t digitval = 0;               // start with digit value 0
    uint16_t divider = DIVIDER[digits]; // read current divider
    while (value >= divider)
    {                   // if current divider fits into the value
      leadflag = 1;     // end of leading spaces
      digitval++;       // increase digit value
      value -= divider; // decrease value by divider
    }
    if (digits == decimal)
      leadflag++; // end leading characters before decimal
    if (leadflag || !lead)
    {
#if OLED_SEG_FONT == 0
