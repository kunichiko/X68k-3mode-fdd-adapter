                               ...
                               uint16_t *ch3_current, uint16_t *ch3_voltage)
{
    ...
    if (ch1_current)
        *ch1_current = conv_current(reg[0]);
    if (ch1_voltage)
        *ch1_voltage = conv_voltage(reg[1]);
    if (ch2_current)
        *ch2_current = conv_current(reg[2]);
    if (ch2_voltage)
        *ch2_voltage = conv_voltage(reg[3]);
    if (ch3_current)
        *ch3_current = conv_current(reg[4]);
    if (ch3_voltage)
        *ch3_voltage = conv_voltage(reg[5]);
}